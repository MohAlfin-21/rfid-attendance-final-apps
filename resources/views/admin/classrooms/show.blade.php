<x-layouts.admin :title="'Detail Kelas'" :subtitle="$classroom->name . ' — ' . $classroom->code">
    <x-slot:actions>
        <a href="{{ route('admin.classrooms.edit', $classroom) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">{{ __('Edit Kelas') }}</a>
    </x-slot:actions>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {{-- Info Card --}}
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h3 class="font-semibold text-gray-800 mb-4">{{ __('Informasi Kelas') }}</h3>
            <dl class="space-y-3 text-sm">
                <div><dt class="text-gray-500">{{ __('Kode') }}</dt><dd class="font-medium font-mono">{{ $classroom->code }}</dd></div>
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <div class="px-6 py-3 bg-gray-50 border-b border-gray-100">
                <form method="POST" action="{{ route('admin.classrooms.update', $classroom) }}" class="flex gap-2">
                    ...
                    <select name="add_student_id" required class="flex-1 rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach($availableStudents as $s)<option value="{{ $s->id }}">{{ $s->name }} ({{ $s->nis ?? $s->username }})</option>@endforeach
                    </select>
                    <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors">{{ __('Tambah') }}</button>
                </form>
            </div>
            @endif
            ...
            <table class="w-full text-sm">
                <thead><tr class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-100">
                    <th class="px-6 py-3">{{ __('Siswa') }}</th><th class="px-6 py-3">NIS</th><th class="px-6 py-3">{{ __('Tahun Ajaran') }}</th><th class="px-6 py-3 text-right">{{ __('Aksi') }}</th>
                </tr></thead>
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="px-6 py-3 text-right">
                            <form method="POST" action="{{ route('admin.classrooms.update', $classroom) }}" onsubmit="return confirm('{{ __('Keluarkan :name dari kelas?', ['name' => $student->name]) }}')">
                                ...
                                <button type="submit" class="text-red-600 hover:text-red-800 text-xs font-medium">{{ __('Keluarkan') }}</button>
                            </form>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="4" class="px-6 py-12 text-center text-gray-400">{{ __('Belum ada siswa di kelas ini') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
