                    ...
                    <tr class="hover:bg-gray-50/60 transition-colors" x-data="{ open: false }">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-7 h-7 bg-gradient-to-br from-sky-500 to-cyan-600 rounded-lg flex items-center justify-center text-white font-bold text-xs shrink-0">{{ strtoupper(substr($req->user?->name ?? 'U', 0, 1)) }}</div>
                                <div>
                        ...
                        <td class="px-6 py-4">
                            @if($req->attachment_path)
                                <a href="{{ route('absence-requests.attachment', $req, false) }}" target="_blank" class="inline-flex items-center gap-1 text-sky-600 hover:text-sky-800 text-xs font-semibold bg-sky-50 px-2 py-1 rounded-lg hover:bg-sky-100 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($req->isPending())
                                <button @click="open = !open" class="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 hover:text-sky-800 bg-sky-50 hover:bg-sky-100 px-3 py-1.5 rounded-lg transition-colors">
                                    Review
                                </button>
                                <div x-show="open" x-cloak x-transition class="mt-3 p-4 bg-gray-50 rounded-xl border border-gray-200 text-left min-w-[220px]">
                                    <form method="POST" action="{{ route('secretary.absence-requests.review', $req) }}" class="space-y-3">
                                        @csrf @method('PUT')
                                        <div>
                                            <label class="block text-xs font-semibold text-gray-500 mb-1">Catatan (opsional)</label>
                                            <textarea name="review_note" placeholder="Tambahkan catatan..." rows="2" class="w-full rounded-xl border-gray-200 text-xs focus:border-sky-500 focus:ring-sky-500"></textarea>
                                        </div>
