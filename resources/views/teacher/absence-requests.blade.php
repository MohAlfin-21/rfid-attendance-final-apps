                ...
                <tr class="hover:bg-gray-50/50" x-data="{ open: false }">
                    ...
                    <td class="px-6 py-3">
                        @if($req->attachment_path)
                            <a href="{{ asset('storage/' . $req->attachment_path) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Lihat</a>
                    ...
                    <td class="px-6 py-3 text-right">
                        @if($req->isPending())
                        <button @click="open = !open" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Review</button>
                        <div x-show="open" x-cloak class="mt-2 p-3 bg-gray-50 rounded-lg text-left">
                            <form method="POST" action="{{ route('teacher.absence-requests.review', $req) }}" class="space-y-2">
                                @csrf @method('PUT')
                                <textarea name="review_note" placeholder="Catatan (opsional)..." rows="2" class="w-full rounded-lg border-gray-300 text-xs focus:border-emerald-500 focus:ring-emerald-500"></textarea>
