    ...
    <div class="h-screen flex overflow-hidden">
        ...
        <aside :class="[sidebarOpen ? 'translate-x-0' : '-translate-x-full', miniSidebar ? 'lg:w-[72px] w-64 mini-sidebar' : 'w-64']" class="fixed inset-y-0 left-0 z-50 bg-slate-950 transform transition-all duration-300 lg:translate-x-0 lg:static lg:inset-auto flex flex-col shadow-2xl overflow-hidden truncate">
            <div class="logo-box flex items-center gap-3 px-6 py-4 border-b border-slate-800 transition-all duration-300 select-none" @click="miniSidebar = !miniSidebar" title="Toggle Sidebar">
                ...
                <div class="sidebar-text">
                    <h1 class="text-white font-bold text-sm leading-tight">Panel Guru</h1><p class="text-slate-400 text-xs">Wali Kelas</p></div>
            </div>
            <nav class="flex-1 px-3 py-3 space-y-1 overflow-y-auto">
                <p class="px-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Menu Utama</p>
                ...
                <p class="px-4 pt-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Kelas Saya</p>
                <a href="{{ route('teacher.classroom') }}" class="{{ $linkBase }} {{ request()->routeIs('teacher.classroom') ? $linkActive : $linkInactive }}">
                ...
                <a href="{{ route('teacher.attendance') }}" class="{{ $linkBase }} {{ request()->routeIs('teacher.attendance') ? $linkActive : $linkInactive }}">
                ...
                <a href="{{ route('teacher.absence-requests') }}" class="{{ $linkBase }} {{ request()->routeIs('teacher.absence-requests*') ? $linkActive : $linkInactive }}">
            ...
            <div class="px-4 py-3 border-t border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0 sidebar-text"><p class="text-sm font-medium text-white truncate">{{ Auth::user()->name }}</p><p class="text-xs text-slate-400 truncate">Guru</p></div>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto w-full">
            ...
            <div class="px-4 lg:px-8">
                @if(session('success'))<div class="mt-4 bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-lg flex items-center gap-2" x-data="{ show: true }" x-show="show" x-transition><svg class="w-5 h-5 text-emerald-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span class="text-sm">{{ session('success') }}</span><button @click="show = false" class="ml-auto text-emerald-400 hover:text-emerald-600">&times;</button></div>@endif
